...
const App: React.FC = () => {
  ...
  const handleCheckIn = async () => {
    ...
    const guest = allGuests.find(g => g.igId === cleanId);
    
    if (!guest) {
      setState(prev => ({ 
        ...prev, 
        isCheckingIn: false, 
        error: "找不到此帳號，請確認試算表內 IG ID 是否正確。" 
      }));
      return;
    }
      ...
      <main className="flex-1 px-5 pb-20 z-10 space-y-8">
        {!state.checkedInGuest ? (
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-right-12 duration-700 ease-out-expo">
            ...
            {state.checkedInGuest.tableInsight && (
            ...
            <section className="space-y-4 py-2">
              <div className="flex items-center justify-between px-3">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-black text-stone-500 uppercase tracking-[0.2em] text-[10px]">Personal Icebreakers</h3>
                </div>
              ...
              <div className="grid gap-3">
                {[
                  { icon: Lightbulb, label: '分享想法', text: state.checkedInGuest.sharing, color: 'text-amber-600', bg: 'bg-amber-50', border: 'border-amber-100/40' },
                  { icon: Compass, label: '集思廣益', text: state.checkedInGuest.brainstorming, color: 'text-emerald-600', bg: 'bg-emerald-50', border: 'border-emerald-100/40' },
                  { icon: Dices, label: '搞怪話題', text: state.checkedInGuest.quirky, color: 'text-rose-500', bg: 'bg-rose-50', border: 'border-rose-100/40' }
                ].map((item, idx) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-stone-100 flex items-center justify-center text-[10px] font-black text-stone-400">
                        {mate.gender === '女' ? '♀' : mate.gender === '男' ? '♂' : mate.name.charAt(0)}
                      </div>
